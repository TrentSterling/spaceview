//! Custom window chrome: SpaceView runs `.with_decorations(false)`, so the OS title
...
//! and resizing it from the edges. Ported from TrontEQ's window_chrome.rs (the
//! campaign pattern; Boxel and TrontSnap have the same treatment).
//!
//! Why: the toolbar already draws the SpaceView wordmark + controls, so a stock
//! Windows caption above it was a second, mismatched bar wasting ~30px.
//!
...
//! Judged not worth a Win32 `WM_NCHITTEST` subclass, same call as the other apps.
//!
//! Close routes through `ViewportCommand::Close` — SpaceView has no tray, so
//! close means quit, which is exactly what that command does.
//!
//! No theme module here (unlike TrontEQ): every color follows `ui.visuals()` so
//! the runtime light/dark toggle just works, with the close-hover red as the one
//! deliberate exception.

// SpaceView has no direct `egui` dependency (only `eframe`, which re-exports
// it), unlike the donor. `self` brings the `egui::` path prefix into scope
// alongside the named imports so the rest of this file can stay verbatim.
...
/// than text so they never depend on font glyph coverage.
...
pub fn caption_overlay(ctx: &egui::Context) {
    ...
    egui::Area::new(egui::Id::new("spaceview_caption_overlay"))
            ...
            egui::Frame::NONE
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        if window_button(ui, WinBtn::Close).on_hover_text("Close").clicked() {
...
pub fn edge_resize(ctx: &egui::Context) {
    ...
    for (i, (rect, dir, cursor)) in strips.into_iter().enumerate() {
        ...
        egui::Area::new(egui::Id::new("spaceview_resize_edge").with(i))
            .order(egui::Order::Foreground)
            .fixed_pos(rect.min)
            .show(ctx, |ui| {
